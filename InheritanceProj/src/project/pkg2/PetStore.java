...
public class PetStore implements PetStoreSpecification{
    ...
    public void adoptionDrive(ArrayList<Pet> pet)
    {
        ...
        {
            if(pet.get(i) instanceof Dog)
            {
                pet.get(i).setID(getNextPetID());
                availableDogs.add((Dog)pet.get(i));
            }
            else if(pet.get(i) instanceof Cat)
            {
                pet.get(i).setID(getNextPetID());
                availableCats.add((Cat)pet.get(i));
            }
            else if(pet.get(i) instanceof ExoticPet)
            {
                pet.get(i).setID(getNextPetID());
                availableExoticPets.add((ExoticPet)pet.get(i));
            }
        }
    }
    
    @Override
    public int inventoryValue(String s)
    {
        if(s.equals("dog"))
        {
            return availableDogs.size();
        }
        else if(s.equals("cat"))
        {
            return availableCats.size();
        }
        return availableExoticPets.size();
